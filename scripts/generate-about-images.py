#!/usr/bin/env python3
"""Generate responsive WebP/AVIF about section variants from source images."""
...
OUT_DIR = ABOUT_DIR / "optimized"
MANIFEST_PATH = ROOT / "src" / "data" / "aboutImageOptimizedManifest.json"

WEBP_QUALITY = 82
AVIF_QUALITY = 80

# About collage + Raising Dave's square media (~22–28rem display).
TARGET_WIDTHS = (720, 1200)

SIZES = "(min-width: 768px) min(42vw, 28rem), min(76vw, 22rem)"
...
def encode_variants(image: Image.Image, base_name: str) -> list[dict]:
    ...
    for width in widths:
        ...
        for fmt, quality in (("webp", WEBP_QUALITY), ("avif", AVIF_QUALITY)):
            ...
            variants.append(
                {
                    "src": f"/images/about/optimized/{filename}",
...
def main() -> None:
    ...
    for source_path in source_files:
        ...
        manifest["images"][base_name] = {
            "source": f"/images/about/{source_path.name}",
            "width": source_width,
