#!/usr/bin/env python3
"""Generate responsive WebP/AVIF menu card variants from source images."""
...
MENU_DIR = ROOT / "public" / "images" / "menu"
OUT_DIR = MENU_DIR / "optimized"
MANIFEST_PATH = ROOT / "src" / "data" / "menuImageOptimizedManifest.json"

WEBP_QUALITY = 82
AVIF_QUALITY = 80

# Menu cards are square; 4-col desktop (~280px), 2-col tablet, ~248px mobile carousel.
TARGET_WIDTHS = (480, 800)

SIZES = "(min-width: 1100px) 25vw, (min-width: 600px) 50vw, min(76vw, 15.5rem)"
...
def encode_variants(image: Image.Image, base_name: str) -> list[dict]:
    ...
    for width in widths:
        ...
        for fmt, quality in (("webp", WEBP_QUALITY), ("avif", AVIF_QUALITY)):
            ...
            variants.append(
                {
                    "src": f"/images/menu/optimized/{filename}",
...
def main() -> None:
    ...
    for source_path in source_files:
        ...
        manifest["images"][base_name] = {
            "source": f"/images/menu/{source_path.name}",
            "width": source_width,
